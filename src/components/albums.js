...
console.log(photos);
    return (
        <div >
            <div style={{ margin: showPhotos ? '0 15% 0 0' : '0', display:'block' }}>
           ...
           {showPhotos && <div style={{ width: '15%', float: 'right' , display:'block', position:'relative', top:'0'}}>
                <button style={{display:'block', border: '0'}} onClick={()=> setShowPhotos(false)}>

                <HighlightOffIcon color="secondary"/>
                </button>
                {photos && photos.slice(photosNum, photosNum + 10).map((item, idx) =>
                    <img key={idx} src={item.url} width={'100px'} />)}
               {showPhotos && <Pagination count={5} color="primary" onClick={(e) => { setPhotosNum((e.target.innerText - 1)*10); }} />}
            </div>}
        </div>
    );
}